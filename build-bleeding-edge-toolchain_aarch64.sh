...
#
# file: build-bleeding-edge-toolchain_aarch64.sh
#
...
if [ 1 = 1 ]; then
    ...
    if [ "${skipGdb}" = "n" ]; then
	download ${expatArchive} https://github.com/libexpat/libexpat/releases/download/$(echo "R_${expatVersion}" | sed 's/\./_/g')/${expatArchive}	download ${gdbArchive} http://ftp.gnu.org/gnu/gdb/${gdbArchive}
    fi
